...
export default function AddFriend() {
  ...
  const getListStrangers = async () => {
    try {
      const { data, error } = await supabase.rpc("get_strangers", {
  ...
  const acceptInvitatinon = async (relation_id) => {
    ...
    const handleSendFriendReq = async (friend_id) => {
      ...
      const { error } = await supabase.from("friends").insert([{ ...req }]);
      if (error) {
        console.error("Supabase error: ", error);
        return;
      }
    ...
    return (
      <SharedLayout
        headerTitle="Bạn bè"
        leftIcon={
          <ReturnBackIcon onPress={() => router.push("/(app)/listFriends")} />
        }
      >
        <View className="flex-1">
          ...
          {friendInvitations.length > 0 && (
            <View>
              <Text className="py-4 text-2xl text-white font-bold">
                {friendInvitations.length} lời mời kết bạn mới
              </Text>
