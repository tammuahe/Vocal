...
export default function ListFriends() {
  ...
  return (
    <SharedLayout
      headerTitle="Bạn bè"
      className="h-screen"
      leftIcon={<UserAddIcon onPress={navigateToAddFriendScreen} />}
      showNavBottom={isShowNavBottom}
    >
      <View className="flex-1">
        <TextInput
          className="w-full bg-white rounded-full px-6 py-4 mt-2 text-base"
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <FriendItem
                className="mb-2"
                infor={item}
                onMoreActionPress={(data) => handlePressFriendMoreAction(data)}
              />
            )}
          />
        </View>
      ...
      {!isShowNavBottom && (
        <Pressable
          onPress={() => handlePressFriendMoreAction(null)}
          className="absolute bottom-0 left-0 right-0 z-40 w-screen h-screen bg-black opacity-30"
        ></Pressable>
      )}
      <Animated.View
      ...
      >
        ...
        <View className="flex-row px-6 pb-2">
          ...
          <View className="flex-col items-start ml-2">
            ...
            <Text className="text-white mt-1">
              Bạn bè từ {converDateToString(infoFriend?.add_at)}
            </Text>
          </View>
        </View>
        ...
        <View className="flex-col items-left py-4 px-6">
          <TouchableOpacity
            ...
            onPress={() => createConversation(friend)}
          >
            <MessageIcon />
            <Text className="ml-2 text-xl text-white">
              Nhắn tin cho {friend?.user_name}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity className="flex-row items-center mb-4">
            <UnfriendIcon />
            <Text className="ml-2 text-xl text-white">
              Hủy kết bạn với {friend?.user_name}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity className="flex-row items-center mb-4">
            <BlockIcon />
            <Text className="ml-2 text-xl text-white">
              Chặn {friend?.user_name}
            </Text>
          </TouchableOpacity>
        </View>
      </Animated.View>
