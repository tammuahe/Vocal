...
export default function InboxBubble(data) {
  ...
  if (isUser) {
    return (
      <View>
        <TouchableOpacity
          className="flex-row items-end ml-auto m-1 mr-0"
          onPress={handlePressMessage}
        >
          <View className="bg-[#B0EEDC] m-1 p-4 py-5 rounded-t-2xl rounded-bl-2xl w-auto block max-w-[80%] mb-0">
            <Text>{messageText}</Text>
            {timeIsVisible && (
              <View className="flex-row items-end ml-auto mr-0 mt-0 mb-1">
                <Text
                  className="font-inter text-[#968F8F]"
      ...
      <View>
        <View className="flex-row m-1">
          ...
          <View className="flex-1">
            ...
            <TouchableOpacity
              className="bg-white m-1 p-4 py-5 rounded-t-2xl rounded-br-2xl w-auto block max-w-[80%] mb-1"
